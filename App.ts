...
type PaletteShape<T extends ColorData> = 
{ // colordata + base tone
    [COLOR in keyof InputModel]:ReturnType<ColorConfig<T>['base']> & InputModel[COLOR]
} & 
{ //tones 
    [COLOR in keyof InputModel as
        (
            {
...
export function createPalette<T extends ColorData>(inputModel : InputModel,  config : ColorConfig<T>) : PaletteShape<T>{
    const result = {} as PaletteShape<T>;

    for (const color in inputModel){
        result[color]={...inputModel[color],...config.base(input[color])}

        for(const toneKey in config.tones){
            const toneCallback = config.tones[toneKey];
            const subtoneName = toneCallback.subtone.name;
            result[`${color}_${subtoneName}`] = toneCallback(input[color]); //конвенция имён говорит о том что у тона должны быть субтоны. (тон называется depths, а надо использовать depth)
            
            for (const sub in toneCallback.subtone.subtone) {
                const subCallback = toneCallback.subtone.subtone[sub];
                result[`${color}_${sub}_${subtoneName}`] = subCallback(input[color]);
            }
...
const testPalette = createPalette(input, {base:baseColors, tones:{brightness:brightness, depths:depths}}) as PaletteShape<ColorData>;
